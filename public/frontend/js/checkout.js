...
$(function() {
	...
	$("body").on('click', '.postage-box', function() {
		...
		if ($(this).data("id") == 1) {
			...
			$(".postage-info-" + shop_id).html(name + '￥' + price);
	...
	$("body").on('click', '.pickup-edit', function() {
		...
		// $(".postage-info-" + $(this).data("shop-id")).html('您可以选择离您最近的自提点上门提货，到店自提免运费');
		$(".bg").show();
	})
	...
	} else if ($(".address-list").find(".address-box").size() == 0) {
	...
	if (inv_type == 1) {
		...
		if (invoice.inv_title != "个人") {
			if ($.trim(invoice.inv_company) == "") {
				...
				$.msg("纳税人识别号不能为空");
				$("#invoice_type_1").find("#inv_taxpayers").focus();
				return;
			} else {
