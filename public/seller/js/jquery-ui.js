...
(function(factory) {
	...
	$.widget("ui.draggable", $.ui.mouse, {
		...
		_blurActiveElement: function(event) {
			...
			// Blur any element that currently has
			// focus, see #4261
	...
	var widgetsMenu = $.widget("ui.menu", {
		...
		_create: function() {
			...
			this._on({
				...
				"mouseenter .ui-menu-item": function(event) {
					...
					// Ignore bubbled events on
					// parent items, see #11641
		...
		_filterMenuItems: function(character) {
			...
			// Only match on items, not dividers
			// or other content (#10571)
	...
	var widgetsControlgroup = $.widget("ui.controlgroup", {
		...
		// To support the enhanced option in jQuery
		// Mobile, we isolate DOM manipulation
		_enhance: function() {
		...
		_initWidgets: function() {
			...
			// First we iterate over each of the items
			// options
		...
		refresh: function() {
			...
			// We filter here because we need to track
			// all childWidgets not just the visible
			// ones
			...
			if (children.length) {
				...
				// We do this last because we need to
				// make sure all enhancment is done
				...
				// Finally call the refresh method on
	...
	$.widget("ui.checkboxradio", [$.ui.formResetMixin, {
		...
		_getCreateOptions: function() {
			...
			// We read the type here, because it
			// makes more sense to throw a
			// element type error first,
			// rather then the error for lack of
			// a label. Often if its the wrong
			// type, it
			// won't have a label (e.g. calling
			// on a div, btn, etc)
			this._readType();

			labels = this.element.labels();

			// If there are multiple labels, use
			// the last one
			...
			// We need to get the label text but
			// this may also need to make sure
			...
			// Set the label option if we found
			// label text
		...
		_getRadioGroup: function() {
			...
			if (this.form.length) {
				...
				// Not inside a form, check all
				// inputs that also are not
				// inside a form
		...
		_setOption: function(key, value) {
			...
			// We don't allow the value to be
			// set to nothing
			if (key === "label" && !value) {
				return;
			}
		...
		_updateLabel: function() {
			...
			// Remove the contents of the label
			// ( minus the icon, icon space, and
			// input )
	...
	$.widget("ui.button", {
		...
		_getCreateOptions: function() {
			...
			// This is to support cases like in jQuery
			// Mobile where the base widget does have
		...
		_create: function() {
			...
			// We have to check the option again here
			// even though we did in _getCreateOptions,
			// because null may have been passed on init
			// which would override what was set in
			// _getCreateOptions
			if (this.options.disabled == null) {
			...
			// Check to see if the label needs to be set
			// or if its already correct
			if (this.options.label && this.options.label !== this.originalLabel) {
				if (this.isInput) {
		...
		_updateIcon: function(option, value) {
			...
			if (!this.icon) {
				...
				// If we are updating the icon remove
				// the old icon class
				this._removeClass(this.icon, null, this.options.icon);
			}

			// If we are updating the icon add the new
			// icon class
			if (icon) {
			...
			// If the icon is on top or bottom we need
			// to add the ui-widget-icon-block class and
			// remove
			...
			if (displayBlock) {
				...
				// Position is beginning or end so
				// remove the ui-widget-icon-block class
				// and add the
		...
		_setOption: function(key, value) {
			...
			// Make sure we can't end up with a button
			// that has neither text nor icon
			if (key === "showLabel") {
			...
			if (key === "label") {
				if (this.isInput) {
					...
					// If there is an icon, append it,
					// else nothing then append the
					// value
					// this avoids removal of the icon
					// when setting label text
		...
		refresh: function() {
			...
			// Make sure to only check disabled if its
			// an element that supports this otherwise
			// check for the disabled class to determine
			// state
	...
	$.widget("ui.spinner", {
		...
		// Support mobile enhanced option and make
		// backcompat more sane
		_enhance: function() {
		...
		_draw: function() {
			...
			// TODO: Right now button does not support
			// classes this is already updated in button
			// PR
