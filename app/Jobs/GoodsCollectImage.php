...
class GoodsCollectImage implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if ($res['code'] == -1) {
            Log::stack(['job'])->info('GoodsCollectImage fail goods_id:'.$goods_id);
            return;
        }
        $path = $res['data']['path'] ?? '';
        DB::table('goods')->where('goods_id', $goods_id)->update(['goods_image'=>$path]);


        Log::stack(['job'])->info('GoodsCollectImage success goods_id:'.$goods_id);
        return;
    }
}
