...
class GoodsCollectVideo implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if ($res['code'] == -1) {
            Log::stack(['job'])->info('GoodsCollectVideo fail goods_id:'.$goods_id);
            return;
        }
        $path = $res['data']['path'] ?? '';
        DB::table('goods')->where('goods_id', $goods_id)->update(['goods_video'=>$path]);


        Log::stack(['job'])->info('GoodsCollectVideo success goods_id:'.$goods_id);
        return;
    }
}
