...
class UserCollectController extends UserCenter
{

    protected $userCollect;

    public function __construct(UserCollectRepository $userCollect)
    {
        parent::__construct();
    ...
    public function lists(Request $request)
    {
        ...
        $where[] = ['user_collect.user_id', $this->user_id];
        ...
        $condition = [
            'join' => [
                [
                    'join_table' => 'article',
                    'join_first' => 'user_collect.target_id',
                    'join_operator' => '=',
            ...
            'where' => $where,
            'sortname' => 'user_collect.collect_id',
            'sortorder' => 'desc',
            'field' => [
                'user_collect.*', 'article.article_id', 'article.title', 'article.article_thumb', 'article.article_type',
                'user.user_id as target_user_id', 'user.nickname as target_nickname', 'user.headimg as target_headimg']
        ];
        list($list, $total) = $this->userCollect->getList($condition);
        if ($list->isNotEmpty()) {
    ...
    public function toggle(Request $request)
    {
        ...
        if ($type == 1) { // 文章
            if ($this->userCollect->checkIsCollected($this->user_id, $target_id, $type)) {
                // 取消关注
                $msg = '取消收藏';
            } else {
                // 关注
            ...
            $ret = $this->userCollect->toggle($this->user_id, $target_id, $type);
        } else {
            // 其他类型 ...
