...
class CollectController extends Backend
{
    ...
    public function show(Request $request)
    {
        ...
        $explain_panel = [
            ...
            '<font color="red">采集机制更换，实时采集在线数据，保证数据的准确性，及时性，同时保证稳定，采集速度有所下降，请谅解！</font>'
        ];
        $blocks = [
...
//            $ret = $this->yunCollectRep->doCollect($collectModel);
            $ret = $this->yunCollectRep->doCollectSzy($collectModel);
            if ($ret === false) {
                flash('error', '采集失败');
        ...
        return view('goods.collect.show', compact('title','lib_category_list','goods_type_list','cat_list','can_collect','collect_msg'));
    }
...
//            $ret = $this->yunCollectRep->doCollectDetail($current_id);
            $ret = $this->yunCollectRep->doCollectSzyDetail($current_id);
