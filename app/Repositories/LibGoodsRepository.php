...
class LibGoodsRepository
{
    ...
    {
        $this->model = new LibGoods();
        $this->libGoodsSku = new LibGoodsSkuRepository();
        $this->category = new CategoryRepository();
        $this->libGoodsAttr = new LibGoodsAttrRepository();
        $this->libGoodsSpec = new LibGoodsSpecRepository();
        $this->libSpecAlias = new LibSpecAliasRepository();
    }
...
//        }
    ...
    {
        $goodsInsert = $postData['LibGoodsModel'];
...
//            $goodsInsert['other_cat_ids'] = !empty($postData['other_cat_ids']) ? implode(',',$postData['other_cat_ids']) : null; // todo 暂时不确定什么意思
            $goodsInsert['contract_ids'] = !empty($goodsInsert['contract_ids']) ? serialize($goodsInsert['contract_ids']) : null;
            $goodsInsert['mobile_desc'] = serialize($goodsInsert['mobile_desc']);
            $goodsInsert['other_attrs'] = !empty($postData['other_attrs']) ? serialize($postData['other_attrs']) : null;  // 平台自定义属性
            ...
            // 设置默认sku_id
            $default_sku_id = LibGoodsSku::where([['goods_id',$goodsRet->goods_id],['checked',1]])->select(['sku_id','goods_id'])->orderBy('sku_id', 'asc')->value('sku_id');
            // 更新商品表 sku_id 为goods_sku 第一个
            LibGoods::where('goods_id', $goodsRet->goods_id)->update(['sku_id'=>$default_sku_id]);
...
//            }
    ...
    public function createGoodsSku($sku_list, $goodsRet)
    {
        if (empty($sku_list)) {
            ...
            $goodsSkuInsert = [
                'goods_id' => $goodsRet->goods_id,
                'market_price' => !empty($goodsRet['market_price']) ? $goodsRet['market_price'] : 0,
                'goods_price' => $goodsRet['goods_price'],
                'goods_number' => !empty($goodsRet['goods_number']) ? $goodsRet['goods_number'] : 0,
                'warn_number' => !empty($goodsRet['warn_number']) ? $goodsRet['warn_number'] : 0,
                'goods_sn' => $goodsRet['goods_sn'] ?? '',
                'goods_barcode' => $goodsRet['goods_barcode'] ?? '',
//                    'goods_stockcode' => !empty($goodsInsert['goods_stockcode']) ? $goodsInsert['goods_stockcode'] : 0,
                'is_spu' => 1, // 无规格商品 是SPU商品
                'sku_name' => $goodsRet->goods_name
            ];
            $goodsSku = new LibGoodsSku();
            ...
            foreach ($sku_list as $skuKey=>$item) {
                ...
                foreach ($item['specs'] as $s) {
                    $spec_ids[] = LibGoodsSpec::where([['goods_id', $goodsRet->goods_id], ['attr_vid', $s['attr_vid']]])->value('spec_id');
    ...
    {
        $goodsUpdate = $postData['LibGoodsModel'];
...
//            $goodsUpdate['other_cat_ids'] = implode(',',$postData['other_cat_ids']); // todo
            ...
            $goodsUpdate['contract_ids'] = !empty($goodsUpdate['contract_ids']) ? serialize($goodsUpdate['contract_ids']) : null;
...
//            }
            ...
            if (!empty($postData['goods_specs'])) {
                ...
                LibGoodsSpec::where('goods_id', $goods_id)->update(['is_checked' => 0]);

                foreach ($postData['goods_specs'] as $gsKey=>$item) {
                    // 检查是否存在
                    $goodsSpecCondition = [['goods_id', $goods_id], ['attr_id', $item['attr_id']], ['attr_vid', $item['attr_vid']]];
                    $isExist = LibGoodsSpec::where($goodsSpecCondition)->get()->toArray();
                    if (count($isExist) > 0) {
                        // 存在 更新数据
                        $goodsSpecUpdate = [
                            'goods_id' => $goods_id,
                        ...
                        LibGoodsSpec::where($goodsSpecCondition)->update($goodsSpecUpdate);
                    } else {
                        // 不存在 新增数据
                        $goodsSpecInsert = [
                            'goods_id' => $goods_id,
            ...
            if (!empty($postData['sku_list'])) {
                ...
                LibGoodsSku::where('goods_id', $goods_id)->update(['checked' => 0]);
                ...
                foreach ($postData['sku_list'] as $item) {
                    ...
                    $spec_ids = LibGoodsSpec::where('goods_id', $goods_id)
                    ...
                    $isExist = LibGoodsSku::where($goodsSkuCondition)->get()->toArray();
                    ...
                    if (count($isExist) > 0) {
                        ...
                        LibGoodsSku::where($goodsSkuCondition)->update($item);
                    } else {
                        // 不存在 新增
                        $goodsSku = new LibGoodsSku();
                        $goodsSku->fill($item);
            ...
            $default_sku_id = LibGoodsSku::where([['goods_id',$goods_id],['checked',1]])->select(['sku_id','goods_id'])->orderBy('sku_id', 'asc')->value('sku_id');
            // 更新商品表 sku_id 为goods_sku 第一个
            LibGoods::where('goods_id', $goods_id)->update(['sku_id'=>$default_sku_id]);


            // 3. 插入商品属性表 goods_attr
            if (!empty($postData['goods_attrs'])) {
                // 删除原有的商品属性
                LibGoodsAttr::where('goods_id', $goods_id)->delete();
                // 插入新的数据
...
//                    }
            ...
            if (!empty($postData['spec_alias'])) {
                // 删除原有的商品规格别名
                LibSpecAlias::where([['goods_id',$goods_id]])->delete();
...
//            echo $e->getCode();
    ...
    {
        $goods_id = DB::table('lib_goods_sku')->where('sku_id', $sku_id)->orderBy('sku_id', 'asc')->value('goods_id');
    ...
    {
        $sku_id = LibGoods::where('goods_id', $goods_id)->select(['goods_id','sku_id'])->value('sku_id');
        return $sku_id;
    }
    ...
    public function getSkuList($goods_id)
    {
        ...
        list($data, $total) = $this->libGoodsSku->getList($condition);
...
//                } else {
//                    // 无规格 获取商品默认主图
//                    $item->sku_image = LibGoodsImage::where('goods_id',$goods_id)->orderBy('is_default', 'asc')->value('path');
//                }
//            }
//        }
    ...
    public function editGoodsSkuInfo(Request $request)
    {
        ...
        try {
            ...
            $sku_info = LibGoodsSku::where('sku_id', $sku_id)->select(['goods_id','goods_price','goods_number'])->first();
            ...
            // 更新goods_sku表中数据
            $this->libGoodsSku->editInfo($request);
    ...
    public function foreverDeleteGoods($shop_id = 0, $goods_ids = [])
    {
        ...
        try {
            ...
            if (!empty($shop_id)) {
                // 删除店铺所有商品
                $goods_ids = LibGoods::where('shop_id', $shop_id)->select(['goods_id'])->pluck('goods_id')->toArray();
            }

            LibGoods::whereIn('goods_id', $goods_ids)->delete(); // 商品表 goods
            LibGoodsAttr::whereIn('goods_id', $goods_ids)->delete(); // 商品属性 goods_attr
//            GoodsCat::whereIn('goods_id', $goods_ids)->delete(); // 商品扩展分类 goods_cat
            LibGoodsImage::whereIn('goods_id', $goods_ids)->delete(); // 商品相册 goods_image
            LibGoodsSku::whereIn('goods_id', $goods_ids)->delete(); // 商品SKU goods_sku
            LibGoodsSpec::whereIn('goods_id', $goods_ids)->delete(); // 商品规格 goods_sku
    ...
    public function getGoodsModelInfo($goods_id)
    {
        $field = ['mobile_price','cost_price','market_price','invoice_type','is_repair','user_discount','stock_mode','top_layout_id','bottom_layout_id','warn_number','goods_number','goods_sort','freight_id','pricing_mode','sales_model','goods_status','goods_id','goods_name','cat_id','cat_id1','cat_id2','cat_id3','shop_id','sku_open','sku_id','goods_subname','goods_price','give_integral','goods_sn','goods_barcode','goods_image',
            ...
            'goods_video','brand_id','pc_desc','mobile_desc','packing_layout_id','service_layout_id','click_count','keywords','goods_info','comment_num','sale_num','collect_num','goods_audit','goods_reason','is_delete','is_virtual','is_best','is_new','is_hot','is_promote','contract_ids','supplier_id',
//            'goods_freight_type','goods_freight_fee','goods_stockcode',
            'goods_volume','goods_weight','goods_remark','add_time','last_time','act_id','lib_cat_id','other_attrs',
//            'shop_cat_ids'
            ];
    ...
    public function getGoodsSpecList($goods_info)
    {
        ...
        list($data, $total) = $this->libGoodsSpec->getList($condition, 'attr_id');
        if (empty($data)) {
            return [];
        }
        $resData = [];
        ...
        foreach ($data as $attr_id=>$item) {
            ...
            foreach ($item as $av) {
                ...
                if ($i == 0) {
                    // 规格缩略图
                    $spec_image = LibGoodsImage::where([['goods_id', $goods_id], ['spec_id', $av->spec_id]])->orderBy('sort', 'asc')->pluck('path')->toArray();
...
//                    'is_invalid' => 0
    ...
    public function getGoodsAttrList($goods_id)
    {
        ...
        list($data, $total) = $this->libGoodsAttr->getList($condition);
        if (empty($data)) {
            return [];
        }
        $resData = [];
    ...
    public function getGoodsImages($goods_id, $spec_id = 0)
    {
        ...
        $goodsImages = LibGoodsImage::where($where)->orderBy('is_default', 'desc')->orderBy('sort', 'asc')->get();
        return $goodsImages;
    }
...
//            $region_code = str_replace('_', ',', $region['region']);
    ...
    {
        $count = LibGoods::where('goods_id',$goodsId)->value('collect_num');
        return $count;
    }
    ...
    public function getOnSaleGoodsInfo($goods_id, $sku_id = 0, $quantity = 0)
    {
        ...
        $where[] = ['goods_audit',1]; // 审核通过
        $where[] = ['goods_id',$goods_id];
        $goods_info = LibGoods::where($where)->first();

        if ($sku_id > 0) {
            $goods_info->sku_id = $sku_id; // 重新赋值 sku_id
            $sku_info = LibGoodsSku::where('sku_id', $sku_id)->first();
            // 默认sku
            ...
            $goods_images = LibGoodsImage::where($where)->orderBy('is_default', 'desc')->orderBy('sort', 'asc')->get();
