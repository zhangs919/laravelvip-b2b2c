...
class LibGoodsImageRepository
{
    ...
    public function addGoodsImage($postGoodsImage, $goods_id)
    {
        ...
        try {
            ...
            if (isset($postGoodsImage['default'])) {
                ...
                // 更新商品SKU表图片
                LibGoodsSku::where('goods_id', $goods_id)
                    ->where('spec_ids', '=', null)
                    ->where('checked', 1)
                    ->update([
    ...
    {
        $goodsSpecs = LibGoodsSpec::where('goods_id', $goods_id)->select(['spec_id'])->get()->toArray();
        $list = [];
        if (!empty($goodsSpecs)) { // 有规格
            foreach ($goodsSpecs as $v) {
                $goodsImages = LibGoodsImage::where([['goods_id',$goods_id],['spec_id',$v['spec_id']]])->orderBy('sort','asc')->pluck('path')->toArray();
            ...
            // 无规格
            $goodsImages = LibGoodsImage::where([['goods_id',$goods_id],['spec_id',0]])->orderBy('sort','asc')->pluck('path')->toArray();
            if (empty($goodsImages)) {
                $goodsImages = ["","","","",""];
            }
