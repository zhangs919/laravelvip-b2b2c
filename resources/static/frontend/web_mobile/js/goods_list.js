$(function() {
	...
	$(".filtrate-term  ul").find("li").click(function() {
		...
		if ($(this).index() == 0) {
			if ($(this).hasClass('active')) {
				...
				tablelist.load(params, $.imgloading.loading, $.loading.stop);
			}

		}
		if ($(this).index() == 1) {
			$.loading.start();
			$("#searchForm").find("input[name='sort']").val(1);
			$("#searchForm").find("input[name='order']").val('DESC');
		...
		if ($(this).index() == 2) {
			...
			$("#searchForm").find("input[name='sort']").val(4);
			if($(this).hasClass('active')){
	...
	$('.filter-clear').click(function() {
		...
		$.each($('#searchForm input'), function() {
			if (!($(this).attr('name') == 'keyword' || $(this).attr('name') == 'cat_id')) {
				$(this).val('');
			}
		});
		$("#slider-range").slider({
