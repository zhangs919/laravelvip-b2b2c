...
(function ($) {
    ...
    function Scroller(elem, settings) {
            ...
            if (event('validate', [dw, index, time]) !== false) {
                ...
                $('.dw-ul', dw).each(function (i) {
                    ...
                    if (!cell.hasClass('dw-v')) {
                        ...
                        // If we have direction (+/- or mouse wheel), the
        ...
        function position(check) {
            ...
            if (s.display == 'modal') {
                ...
                if ((t < st) || (at > st + wh)) { // if doesn't fit above or
		 ...
		 * 
		 * @param {Object}
		 *            t - Target wheel jQuery object.
		 * @param {Number}
		 *            index - Index of the changed wheel.
		 * @param {Number}
		 *            val - Value.
		 * @param {Number}
		 *            time - Duration of the animation, optional.
		 * @param {Number}
		 *            orig - Original value.
		 */
        ...
        /**
		 * Gets the selected wheel values, formats it, and set the value of the
		 * scroller instance. If input parameter is true, populates the
		 * associated input element.
		 * 
		 * @param {Boolean}
		 *            sc - Scroll the wheel in position.
		 * @param {Boolean}
		 *            fill - Also set the value of the associated input element.
		 *            Default is true.
		 * @param {Number}
		 *            time - Animation time
		 * @param {Boolean}
		 *            temp - If true, then only set the temporary value.(only
		 *            scroll there but not set the value)
		 */
        ...
        /**
		 * Checks if the current selected values are valid together. In case of
		 * date presets it checks the number of days in a month.
		 * 
		 * @param {Number}
		 *            time - Animation time
		 * @param {Number}
		 *            orig - Original value
		 * @param {Number}
		 *            i - Currently changed wheel index, -1 if initial
		 *            validation.
		 * @param {Number}
        ...
        that.tap = function (el, handler) {
            ...
            if (s.tap) {
                el.bind('touchstart', function (e) {
                    ...
                    // If movement is less than 20px, fire the click event
					// handler
		 ...
		 * 
		 * @param {Boolean}
		 *            prevAnim - Prevent animation if true
		 */
        that.show = function (prevAnim) {
            ...
            if (s.display != 'inline') {
                ...
                $(window).bind('resize.dw', function () {
                    // Sometimes scrollTop is not correctly set, so we wait a
					// little
            ...
            dw.delegate('.dwwl', 'DOMMouseScroll mousewheel', function (e) {
                ...
                if (testTouch(e) && !move && !isReadOnly(this) && !click) {
                    ...
                    pos = +target.data('pos');
                    setGlobals(target);
                    moved = iv[index] !== undefined; // Don't allow tap, if
														// still moving
                    start = getCoord(e, 'Y');
                    startTime = new Date();
                    stop = start;
                ...
                for (i = 0; i < w.length; i++) {
                    for (l in w[i]) {
                        if (w[i][l][val[j]] !== undefined) {
                            ...
                            for (v in w[i][l]) { // Select first value from
													// wheel
                                ret.push(v);
                                break;
                            }
    ...
    }).bind('mouseover mouseup mousedown click', function (e) { // Prevent
																// standard
																// behaviour on
																// body click
        if (tap) {
