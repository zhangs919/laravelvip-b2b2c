	...
	$.modal = function(options) {
		...
		if (options instanceof jQuery) {
			...
			if ($(element).hasClass(".modal")) {
				return $(element).data("modal-options");
			} else if ($(element).parents(".modal").size() > 0) {
				return $(element).parents(".modal").data("modal-options");
			} else {
				return $(element).data("modal-options");
			}
		}
		...
		var defaults = {
			...
			render: function(content) {
				...
				$(this.container).modal("toggle");
				$(this.container).data("modal-options", this);
				if (this.trigger) {
					$(this.trigger).data("modal-options", this);
				}
