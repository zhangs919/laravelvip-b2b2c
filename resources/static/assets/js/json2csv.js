(function( global, factory ) {
  ...
  var JSonToCSV = {
     ...
     * ## columns 是表头对象，且title和key必须一一对应，包含有
          ...
          append: function() // 自定义拼接方式，传入 (row, key, value)
     */
      ...
      data.map(function(n) {
          ...
          if (columns.key.length) {
              columns.key.map(function(m) {
                  var value = (typeof columns.formatter === 'function' ? columns.formatter(m, n[m]) || n[m] : n[m]);
                  value = typeof value == "undefined" ? '' : value;
                  value = value.toString().replace(/\"/g,"'");
                  // 数据特殊字符去除（目前去除制表符\t,如有新增可以增加）
                  value = value.toString().replace(/[\t]/g,"");
                  if(typeof columns.append === 'function') {
                      row = columns.append(row, m, value);
                  }else{
                      row += '"' + value + '",';
                  }
              });
              ...
              for (key in n) {
                  ...
                  var value = (typeof columns.formatter === 'function' ? columns.formatter(key, n[key]) || n[key] : n[key]);

                  if(typeof columns.append === 'function') {
                      row = columns.append(row, key, value);
                  }else{
                      row += '"' + value + '",';
                  }
              }
