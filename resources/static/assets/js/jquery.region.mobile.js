	...
	$.fn.regionchooser = function(settings) {
		...
		var defaults = {
			...
			load: function(region_code, value_mode, target) {
				...
				region_code = $.grep(region_code, function(code, i) {
					return code != '00';
				});
			...
			render: function(list, value_mode, target) {
				...
				if (list == undefined) {
					...
					html += '</div>';
					html += '</div>';
					html += '<section class="f_block region-chooser-box"  id="region-chooser-box">';
			...
			// 显示
			show: function() {
				$(this.container).find(".region-chooser-box").addClass('region-chooser-show');
				$(".mask-div").show();
				scrollheight = $(document).scrollTop();
				$("body").css("top", "-" + scrollheight + "px");
				$("body").addClass("visibly");
				setTimeout(function() {
					$('.region-chooser-close').addClass('show');
				}, 500);
				// 关闭回调函数
			...
			hide: function() {
				...
				$(this.container).find(".region-chooser-box").removeClass('region-chooser-show');
			...
			toggle: function() {
				...
				if ($(container).find(".region-chooser-box").hasClass('region-chooser-show')) {
					this.hide();
				} else {
					this.show();
				}
			...
			setLabel: function(label) {
				...
				$(this.container).find(".region-chooser-selected").find(".region").find("font").html(label)
			},
			// 显示事件
			show_callback: null,
			// 隐藏事件
