/*******************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************
 * KindEditor - WYSIWYG HTML Editor for Internet Copyright (C) 2006-2016 kindsoft.net
 * 
 ...
 * @version szy-4.1.11 (2016-09-23)
 * @author niqingyang <niqy@qq.com>
 * @version szy-4.1.11 (2017-11-15)
 ******************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************/
(function(window, undefined) {
	...
	var _VERSION = '4.1.11 (2016-03-31)', _ua = navigator.userAgent.toLowerCase(), _IE = _ua.indexOf('msie') > -1 && _ua.indexOf('opera') == -1, _NEWIE = _ua.indexOf('msie') == -1 && _ua.indexOf('trident') > -1, _GECKO = _ua.indexOf('gecko') > -1 && _ua.indexOf('khtml') == -1, _WEBKIT = _ua.indexOf('applewebkit') > -1, _OPERA = _ua.indexOf('opera') > -1, _MOBILE = _ua.indexOf('mobile') > -1, _IOS = /ipad|iphone|ipod/.test(_ua), _QUIRKS = document.compatMode != 'CSS1Compat', _IERANGE = !window.getSelection, _matches = /(?:msie|firefox|webkit|opera)[\/:\s](\d+)/.exec(_ua), _V = _matches ? _matches[1] : '0', _TIME = new Date().getTime();
	...
	K.options = {
		...
		htmlTags: {
			...
			table: ['id', 'class', 'border', 'cellspacing', 'cellpadding', 'width', 'height', 'align', 'bordercolor', '.padding', '.margin', '.border', 'bgcolor', '.text-align', '.color', '.background-color', '.font-size', '.font-family', '.font-weight', '.font-style', '.text-decoration', '.background', '.width', '.height', '.border-collapse','style'],
			...
			img: ['id', 'class', 'src', 'width', 'usemap', 'height', 'border', 'alt', 'title', 'align', '.width', '.height', '.border'],
			...
			video: ['width', 'height', 'style', 'controls', 'poster', 'autoplay', 'src', 'loop', 'id', 'class', 'src', 'width', 'height', 'type', 'loop', 'autostart', 'quality', '.width', '.height', 'align', 'allowscriptaccess', 'wmode'],
			source: ['src', 'type'],
			map: ['id', 'name', 'class'],
			area: ['shape', 'coords', 'class', 'href', 'target'],
			style: ['type', 'id'],
		},
	...
	function _mediaImg(blankPath, attrs) {
		var width = attrs.width, height = attrs.height, type = attrs.type || _mediaType(attrs.src), style = '', srcTag = '';
...
/*******************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************
 * KindEditor - WYSIWYG HTML Editor for Internet Copyright (C) 2006-2011 kindsoft.net
 * 
 * @author Roddy <luolonghao@gmail.com>
...
KindEditor.lang({
	...
	invalidWidth: "宽度必须为整数。",
	invalidHeight: "高度必须为整数。",
	invalidBorder: "边框必须为整数。",
...
/*
 * 如何实现真正的自动高度？ 修改编辑器高度之后，再次获取body内容高度时，最小值只会是当前iframe的设置高度，这样就导致高度只增不减。 所以每次获取body内容高度之前，先将iframe的高度重置为最小高度，这样就能获取body的实际高度。 由此就实现了真正的自动高度 测试：chrome、firefox、IE9、IE8
 */
...
KindEditor.plugin('filemanager', function(K) {
	var self = this, name = 'filemanager', fileManagerJson = K.undef(self.fileManagerJson, '/site/image-gallery.html'), lang = self.lang(name + '.'), lastUuid = 0;

	self.plugin.filemanagerDialog = function(options) {
		var width = K.undef(options.width, 650), height = K.undef(options.height, 510);
		var dirName = K.undef(options.dirName, '');
		var viewType = K.undef(options.viewType, 'VIEW').toUpperCase();
		var clickFn = options.clickFn;

		var uuid = (new Date()).getTime() * 1000 + (lastUuid++) % 1000;

		var html = ['<div id="' + uuid + '" style="padding:10px 20px;">', '</div>'].join('');
		var dialog = self.createDialog({
			name: name,
			width: width,
			height: height,
 ...
 * @licence http://www.kindsoft.net/license.php
 ******************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************/
KindEditor.plugin('videomanager', function(K) {
	var self = this, name = 'videomanager', videoManagerJson = K.undef(self.videoManagerJson, '/site/video-gallery.html'), lang = self.lang(name + '.'), lastUuid = 0;

	self.plugin.videomanagerDialog = function(options) {
		var width = K.undef(options.width, 650), height = K.undef(options.height, 510);
		var dirName = K.undef(options.dirName, '');
		var viewType = K.undef(options.viewType, 'VIEW').toUpperCase();
		var clickFn = options.clickFn;
		...
		// 图片空间
		$("#" + uuid).videogallery({
			url: videoManagerJson,
			// 点击图片事件函数
			click: function(target, path) {
				var data = $(target).data();
				clickFn(data.url, "", data);
			},
			// Ajax提交参数
			data: {
				output: true,
...
KindEditor.plugin('link', function(K) {
	...
	self.plugin.link = {
		edit: function() {
			var lang = self.lang(name + '.'), html = '<div style="padding:20px;">' + '<div class="ke-dialog-row">' + '<label for="keUrl" style="width:60px;">' + lang.url + '</label>' + '<input class="ke-input-text" type="text" id="keUrl" name="url" value="" style="width:260px;" /></div>' + '<div class="ke-dialog-row"">' + '<label for="keType" style="width:60px;">' + lang.linkType + '</label>' + '<select id="keType" name="type"></select>' + '</div>' + '</div>', dialog = self.createDialog({
				name: name,
				width: 450,
				title: self.lang(name),
				body: html,
...
KindEditor.plugin('media', function(K) {
	...
	self.plugin.media = {
		edit: function() {
			var html = ['<div style="padding:20px;">', '<div class="ke-dialog-row">', '<label for="keUrl" style="width:60px;">' + lang.url + '</label>', '<input class="ke-input-text" type="text" id="keUrl" name="url" value="" style="width:160px;" /> &nbsp;', '<input type="button" class="ke-upload-button" value="' + lang.upload + '" /> &nbsp;', '<span class="ke-button-common ke-button-outer">', '<input type="button" class="ke-button-common ke-button" name="viewServer" value="' + lang.viewServer + '" />', '</span>', '</div>', '<div class="ke-dialog-row">', '<label for="keWidth" style="width:60px;">' + lang.width + '</label>', '<input type="text" id="keWidth" class="ke-input-text ke-input-number" name="width" value="550" maxlength="4" />', '</div>', '<div class="ke-dialog-row">', '<label for="keHeight" style="width:60px;">' + lang.height + '</label>', '<input type="text" id="keHeight" class="ke-input-text ke-input-number" name="height" value="400" maxlength="4" />', '</div>', '<div class="ke-dialog-row">', '<label style="width: 60px;">设置</label>', '<input type="checkbox" id="keAutostart" name="autostart" value="" /> ', '<label for="keAutostart" title="设置视频是否自动播放，如果不自动播放建议让视频显示控件，否则客户可能无法观看视频">' + lang.autostart + '</label>', '<input type="checkbox" id="keControls" name="controls" value="" /> ', '<label for="keControls" title="用于控制是否显示视频插件上的控件，如果视频不是自动播放，建议显示控件">' + lang.controls + '</label>', '<input type="checkbox" id="keLoop" name="loop" value=""/>', '<label for="keLoop" title="用于控制视频是否循环播放">' + lang.loop + '</label>', '</div>', '</div>'].join('');
			var dialog = self.createDialog({
				name: name,
				width: 450,
				height: 230,
			...
			}), div = dialog.div, urlBox = K('[name="url"]', div), viewServerBtn = K('[name="viewServer"]', div), widthBox = K('[name="width"]', div), heightBox = K('[name="height"]', div), autostartBox = K('[name="autostart"]', div), controlsBox = K('[name="controls"]', div), loopBox = K('[name="loop"]', div);
			urlBox.val('http://');
			if (allowMediaUpload) {
				var uploadbutton = K.uploadbutton({
					button: K('.ke-upload-button', div)[0],
			...
			if (allowFileManager) {
				viewServerBtn.click(function(e) {
					self.loadPlugin('videomanager', function() {
						self.plugin.videomanagerDialog({
							viewType: 'LIST',
							dirName: 'media',
							clickFn: function(url, title, data) {
								if (self.dialogs.length > 1) {
									...
									if(data && data.width > 0 && data.height > 0){
										...
										widthBox.val(width || K.removeUnit(img.css('width')) || attrs.width || 0);
										heightBox.val(height || K.removeUnit(img.css('height')) || attrs.height || 0);
									}
			...
			if (img) {
				...
				controlsBox[0].checked = (attrs.controls === 'true');
				loopBox[0].checked = (attrs.loop === 'true');
			}else{
				controlsBox[0].checked = true;
			}
			urlBox[0].focus();
			urlBox[0].select();
		},
...
(function() {
	...
	/***************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************
	 * Flash Event Interfaces These functions are used by Flash to trigger the various events. All these functions a Private. Because the ExternalInterface library is buggy the event calls are added to a queue and the queue then executed by a setTimeout. This ensures that events are executed in a determinate order and that the ExternalInterface bugs are avoided.
	 **************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************/
	...
	/***************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************
	 * Debug Console The debug console is a self contained, in page location for debug message to be sent. The Debug Console adds itself to the body if necessary. The console is automatically scrolled as messages appear. If you are using your own debug handler or when you deploy to production and have debug disabled you can remove these functions to reduce the file size and complexity.
	 **************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************/
...
(function() {
	/*
	 * Queue Plug-in Features: Adds a cancelQueue() method for cancelling the entire queue. All queued files are uploaded when startUpload() is called. If false is returned from uploadComplete then the queue upload is stopped. If false is not returned (strict comparison) then the queue upload is continued. Adds a QueueComplete event that is fired when all the queued files have finished uploading. Set the event handler with the queue_complete_handler setting.
	 */
...
KindEditor.plugin('table', function(K) {
	...
	self.plugin.table = {
		...
		rowmerge: function() {
			var table = self.plugin.getSelectedTable()[0], row = self.plugin.getSelectedRow()[0], cell = self.plugin.getSelectedCell()[0], rowIndex = row.rowIndex, nextRowIndex = rowIndex + cell.rowSpan, nextRow = table.rows[nextRowIndex];
		...
		colmerge: function() {
			var table = self.plugin.getSelectedTable()[0], row = self.plugin.getSelectedRow()[0], cell = self.plugin.getSelectedCell()[0], rowIndex = row.rowIndex, cellIndex = cell.cellIndex, nextCellIndex = cellIndex + 1;
...
KindEditor.plugin('template', function(K) {
	...
	self.clickToolbar(name, function() {
		var lang = self.lang(name + '.'), arr = ['<div style="padding:10px 20px;">', '<div class="ke-header">', '<div class="ke-left">', lang.selectTemplate + ' <select>'];
		K.each(lang.fileList, function(key, val) {
			arr.push('<option value="' + key + '">' + val + '</option>');
		});
		html = [arr.join(''), '</select></div>', '<div class="ke-right">', '<input type="checkbox" id="keReplaceFlag" name="replaceFlag" value="1" /> <label for="keReplaceFlag">' + lang.replaceContent + '</label>', '</div>', '<div class="ke-clearfix"></div>', '</div>', '<iframe class="ke-textarea" frameborder="0" style="width:458px;height:260px;background-color:#FFF;"></iframe>', '</div>'].join('');
		var dialog = self.createDialog({
			name: name,
			width: 500,
			title: self.lang(name),
