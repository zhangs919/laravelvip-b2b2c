 ...
 *       ===========热区插件图片dom结构如下(显示的属性为必须)：================= <div
 *       class="hot_area"> 1.图片展示部分： <div class="" name="imageMap"
 *       id="image_map"> <img src="" ref="imageMap" id="photo"/> </div>
 *       2.添加热区渲染部分(表格样式选1，行样式选2)： 1)table body样式： 2)ul样式： <table><tbody
 *       id="areaItems"> </tbody></table>
 *       <ul id="areaItems">
 *       </ul>
 *       3.添加热区按钮部分：
 *       <p>
 *       <a id="add_hot_area" href="javascript:;" class="">添加热区</a>
 *       </p>
 *       4.热区数据存储（隐藏）： <input type="hidden" class="" id="hotAreas"
 *       name="hotAreas" value=""> 5.可添加热区数量与还可添加热区数量实时显示：
 *       <p>
 *       <span class="">已添加已添加<b class="added_amount">0</b>个热区，还可添加<b
 *       class="remain_amount">X</b>个热区</span>
 *       </p>
 *       6.图片url： <input type="text" id="picUrl" name="" class=""/> </div>
 * 
 * ======================setting 属性设置 ：========================== 属性：
 * 1.（必须）maxAmount 最大值设定 2.（必须）tag 行dom标签，目前仅可使用tr,li 3.（必须）params
 * 特殊热区参数名称对象，对象形式：setting.params =
 * {'areaLink':'添加热区时的默认值','areaType':'添加热区时的默认值'};
 * 
 * 回调函数： 4.（必须）domCallBack(params) 特殊dom创建回调函数（必须）， 返回特殊热区dom结构函数,必须是
 * <tr></tr>
 * 或
 * <li></li>
 * ；params：特殊对象params并包含当前行数params.index； 5.（可选）initCallBack(params)
 * 初始化行回调函数（可选），初始化特殊热区函数；params：特殊对象params并包含当前行数params.index；
 * 6.（可选）deleteCallBack(_this,index)
 * 删除行操作回调函数（可选），删除特殊热区函数：_this：删除行tr或li对象，index：删除行数；
 * 7.（必须）readjustCallBack(_this,index)
 * 重新调整剩余行回调函数（必须），删除热区后重新调整剩余列表行数据函数：_this：重新调整每次遍历的行tr或li对象，index：当前行数
 * 
 * =======================热区通用dom获取 ：===========================
 * 
 * 1.areaSort 热区排序号class 2.areaTitle 热区名称class 3.areaMapInfo 热区坐标class
 ...
 * 而在加载渲染时，也是根据此参数的值来加载热区信息，例如： "hotAreas" : "[{'areaTitle':'热区
 * 1','areaLink':'','areaMapInfo':'0,0,90,30'}, {'areaTitle':'热区
 * 2','areaLink':'','areaMapInfo':'260,13,353,112'}]"
 * 4.通用插件依赖了storeBaseUtils.js，该js为弹窗通用js，可删除，并同时修改baseUtils.showNormalDialog方法
 * 5.删除热区后，
 * 会重新调整热区数量及显示数字，也会遍历剩余tr或li，调整名称及热区序号，readjustCallBack用于遍历循环中对当前行的特殊dom的处理
 */
...
$.fn.imageMaps = function(setting) {
	...
	function initAddClickEvent() {
		...
		$("#add_hot_area").unbind("click").click(function() {
			...
			if ($("#image_map").find('img').attr('src') == '') {
				/*
				 * dialogUtil.showNormalDialog({ "title" : "温馨提示", "content" :
				 * '请先上传图片', "showCancelBtn" : false });
				 */
				alert('请先上传图片');
				return;
			}
			...
			if (rowCount != null) {
				...
				if (c == setting.maxAmount) {
					/*
					 * dialogUtil.showNormalDialog({ "title" : "温馨提示", "content" :
					 * '只能添加'+ setting.maxAmount+ '个热区', "showCancelBtn" : false
					 * });
					 */
	...
	this.each(function() {
		...
		if (hotImage != null) {
			...
			var imageContainer = hotImage.parent(),
				imgConrainerOffset = imageContainer.offset(),
				imgOffset = hotImage.offset();
			...
			if (areaInfoJson != null && areaInfoJson.length > 0) {
				for (var j = 0; j < areaInfoJson.length; j++) {
					...
					if (areaMapInfo) {
						...
						$('#areaItems').append(createAreaItem(index, areaTitle, areaLink, areaMapInfo, setting.tag, setting.domCallBack(areaLinkType,areaLink)));
						if (initFlag) {
							setting.initCallBack(setting.params);
						}
	...
	// done
	function createAreaItem(index, areaTitle, areaLink, areaMapInfo, rowTagDom, specialHotItemDom) {
		...
		if (rowTagDom == "tr") {
			...
			item.push('<td class="w70"><span class="areaSort"><b>热区' + index + '</b></span></td>');
			item.push('<td class=""><input type="hidden" class="ipt_border area_title" value="' + areaTitle + '"/></td>');
			//item.push('<td class="w400">链接：<input type="text" class="areaLinkInfo form-control w120" value="' + areaLink + '" /></td>');
			item.push(specialHotItemDom);
			item.push('<div class="hidden"><span class="p-t-5">位置：</span><input type="text" class="areaMapInfo form-control w120" value="' + areaMapInfo + '" /></div></td>');
			item.push('<td class="w50 text-c"><a href="javascript:;" class="second_btn ml delete c-red">删除</a></td>');
			item.push('</tr>');
		} else {
	...
	function bindMapEvent() {
		$('.drag-img-bg').unbind("mousemove");
		$('.drag-img-bg').unbind("mouseup");
		$('.position_container .map_position .map_position_bg').each(function() {
			...
			$('.drag-img-bg').mousemove(function(event) {
...
// 获取相应行的链接
imageMaps.getAreaLink = function(item) {
	return item.find('.areaMapLink').val();
}
...
/**
 * 1.非等比缩放——定宽比，定高比 pic：图片url setting：热区参数对象 proportionWidth：缩放比例 0-1
 * proportionHeight：缩放比例 0-1 imageMapsSwitch ：热区插件开关：true或不填：开，false：关
 */
...
/**
 * 2.非等比缩放——定宽，定高 pic：图片url setting：热区参数对象 scaleWidth： 宽度 scaleHeight：高度
...
/**
 * 3.等比缩放——定宽比 pic：图片url setting：热区参数对象 proportion：缩放比例 0-1 imageMapsSwitch
 * ：热区插件开关：true或不填：开，false：关
 */
...
/**
 * 4.等比缩放——定宽 pic：图片url setting：热区参数对象 scaleWidth：缩放后的宽度 imageMapsSwitch
 * ：热区插件开关：true或不填：开，false：关
 */
...
/**
 * 5.非等比缩放——定宽，定高比 pic：图片url setting：热区参数对象 scaleWidth：缩放后的宽度
 * proportionHeight：高度缩放比例 0-1 imageMapsSwitch ：热区插件开关：true或不填：开，false：关
 */
...
/**
 * 6.等比缩放——自动 pic：图片url setting：热区参数对象 imageMapsSwitch ：热区插件开关：true或不填：开，false：关
 */
...
/**
 * 6.不进行缩放 pic：图片url setting：热区参数对象 imageMapsSwitch ：热区插件开关：true或不填：开，false：关
 */
