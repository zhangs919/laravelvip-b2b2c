		...
		prototype: {
			...
			element: function(element, rules, messages) {
				...
				if (checkElement === undefined) {
					...
					rs = this.check(checkElement, rules, messages) !== false;
					result = result && rs;
					if (rs) {
			...
			check: function(element, rules, messages) {
				...
				var rules = rules || $(element).rules(), rulesCount = $.map(rules, function(n, i) {
					return i;
				...
				for (method in rules) {
					rule = {
						...
						message: messages && messages[method] ? messages[method] : null
					};
					try {
