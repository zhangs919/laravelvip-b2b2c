<div id="{{ $uuid }}" class="p-t-20">
    <div class="table-content m-t-10">
        <!-- 搜索 -->
        <form id="GoodsModel" class="form-horizontal" name="GoodsModel" action="/goods/list/set-price?ids={{ $goods_ids }}" method="POST">
            @csrf
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <div class="form-control-box">
                            ...
                            <input type="text" id="value" class="form-control small m-l-20 m-r-5 start-num" value="0" data-rule-callback="start_number_callback">
                        </div>
                        <input type='hidden' id='goods_ids' value={{ $goods_ids }} />
                        <div class="member-handle-error"></div>
                        <div class="help-block help-block-t">
                            <div class="help-block help-block-t">[+-*/]表示算法，如原价调高2倍，则写成*2</div>
                        </div>
                    </div>
                </div>
            </div>
        </form>	<div class="layui-layer-btn layui-layer-btn-">
            <a class="layui-layer-btn0" id="btn_next_step">确定</a>
...
<script type='text/javascript'>
    ...
    //点击下一步
    $().ready(function() {
        var validator = $("#{{ $uuid }}").find("#GoodsModel").validate();
// 验证规则，此验证规则会影响编辑器中JavaScript的的格式化操作
        ...
        $("#{{ $uuid }}").find("#btn_next_step").click(function() {
            ...
            if ($("#{{ $uuid }}").find(".start-num").valid() == false) {
                return;
            }
