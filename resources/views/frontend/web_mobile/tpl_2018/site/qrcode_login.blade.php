...
<div id="index_content">
    <header class="header-top-nav">
        <div class="header">
            ...
            <div class="header-middle">扫码登录</div>
    ...
    <div class="qrcode-login-con">
        <div class="qrcode-login-pic">
            <img src="/images/qrcode-login-pic.png" />
        </div>
        <p class="qrcode-login-des">{{ sysconf('site_name') }}电脑端登录确认</p>
        <div class="qrcode-login-bottom">
            <form id="login_form" action="/site/qrcode-login.html" method="POST">
                @csrf
                <input type="hidden" name="key" value="{{ $key }}">
                <input type="hidden" name="handle" value='login'>
                <a href="javascript:void(0);" class="btn bg-color" id="btn_submit">确认</a>
                <a href="javascript:void(0);" class="btn" id="captcha_cancel">取消</a>
            </form>
        </div>
    </div>
...
<script>
    $().ready(function() {
        ...
        $("#captcha_cancel").click(function() {
            $("#login_form").find('input[name="handle"]').val('cancel');
            $("#login_form").submit();
            return false;
        });
        // 清除模板缓存
