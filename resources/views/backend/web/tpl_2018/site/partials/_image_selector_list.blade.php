<div id="tablelist" class="attachment-box">
    ...
    <ul class="image-list">
        ...
        <li class="image-item" title="上传时间：{{ $v->created_at }}" data-id="{{ $v->img_id }}">
            <img class="image-box" src="{{ get_image_url($v->path,1) }}?x-oss-process=image/resize,m_pad,limit_0,h_120,w_120" data-path="{{ $v->path }}" data-url="{{ get_image_url($v->path,1) }}" data-width="{{ $v->width }}" data-height="{{ $v->height }}" />
