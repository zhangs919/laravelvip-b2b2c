...
<div class="drop-item {{ $is_valid != '1' ? 'invalid' : ''}}" id='{{ $uid }}' data-tpl_id='' data-shop_id=''
     data-tpl_name='{{ $tpl_name }}' data-is_valid='{{ $is_valid }}' style="width: 100%">

    <!-- banner模块 _start -->
    <div class="banner" style='height: {{ $bg_height }}px;'>

        @if($tpl_name != '' && $is_design)
            <a title="编辑" href="javascript:void(0)" class="selector pic-selector SZY-TPL-SELECTOR" data-uid="{{ $uid }}"
               data-cat_id="1" data-type="3" data-number="5">
            ...
            <ul id="fullScreenSlides" class="full-screen-slides" style="height: {{ $bg_height }}px;">
                @foreach($data['3-1'] as $k=>$v)
                    <li @if($k == 0) style="display:list-item;" @else style="display:none;" @endif >
                        <a href="{{ $v['link'] ?? 'javascript:void(0)' }}" target="_blank" title=""
                           style="background:url({{ get_image_url($v['path']) }}) no-repeat center center; background-size: auto 100%; height:{{ $bg_height }}px;"></a>
                    </li>
                @endforeach
            </ul>
